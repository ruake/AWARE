...
const KpiCard = memo(function KpiCard({
  ...
  const colors = {
    sky: {
      bg: "bg-gcp-blue/10",
      border: "border-gcp-blue/20",
      icon: "text-gcp-blue",
      iconBg: "bg-gcp-blue/15",
      bar: "bg-gcp-blue",
      glow: "rgba(66,133,244,0.25)",
      gradient: "from-gcp-blue/40 via-gcp-blue to-gcp-blue/40",
    },
    rose: {
      bg: "bg-gcp-red/10",
      border: "border-gcp-red/20",
      icon: "text-gcp-red",
      iconBg: "bg-gcp-red/15",
      bar: "bg-gcp-red",
      glow: "rgba(234,67,53,0.25)",
      gradient: "from-gcp-red/40 via-gcp-red to-gcp-red/40",
    },
    emerald: {
      bg: "bg-gcp-green/10",
      border: "border-gcp-green/20",
      icon: "text-gcp-green",
      iconBg: "bg-gcp-green/15",
      bar: "bg-gcp-green",
      glow: "rgba(52,168,83,0.25)",
      gradient: "from-gcp-green/40 via-gcp-green to-gcp-green/40",
    },
    amber: {
      bg: "bg-gcp-yellow/10",
      border: "border-gcp-yellow/20",
      icon: "text-gcp-yellow",
      iconBg: "bg-gcp-yellow/15",
      bar: "bg-gcp-yellow",
      glow: "rgba(251,188,5,0.25)",
      gradient: "from-gcp-yellow/40 via-gcp-yellow to-gcp-yellow/40",
    },
  };
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r ${c.gradient} opacity-60`}
      />
      <div
...
export default function Dashboard() {
  ...
  const kpis = useMemo(() => {
    ...
    const avgPass = deferredRuns.length
      ? Math.round(deferredRuns.reduce((s, r) => s + r.passPct, 0) / deferredRuns.length)
      : 0;
    ...
    <PageWrapper className="proof-page space-y-8">
      ...
      >
          ...
          <KpiCard
            ...
            sub={
              deferredRuns.length > 0
                ? `${deferredRuns.filter((r) => r.env === "QA").length} QA · ${deferredRuns.filter((r) => r.env === "UAT").length} UAT · ${deferredRuns.filter((r) => r.env === "PROD").length} PROD`
                : undefined
            }
          />
        </motion.div>
        <motion.div variants={fadeUp}>
          <KpiCard
            ...
            color="rose"
            sub={kpis.totalFailures > 0 ? "across all runs" : "clean run history"}
          />
        </motion.div>
        <motion.div variants={fadeUp}>
          <KpiCard
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gcp-text-muted">
            Environment Health
          </h2>

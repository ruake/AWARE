...
const QUICK_ACTIONS = [
  ...
  { icon: GitCompare, label: "Compare two runs", shortcut: "C", color: "var(--proof-text-secondary)" },
...
function StatusDot({ status }: { status?: string | null }) {
  ...
  const colors: Record<string, string> = { pass: "var(--proof-green)", fail: "var(--proof-red)", flaky: "var(--proof-yellow)" };
  return <span style={{ width: 8, height: 8, borderRadius: "50%", display: "inline-block", flexShrink: 0, backgroundColor: colors[status] ?? "var(--proof-grey)" }} />;
}
    ...
    <AppLayout activeHref="/search">
      <div style={{ height: "calc(100vh - 100px)", display: "flex", flexDirection: "column", maxWidth: 960, margin: "0 auto" }}>
        <div className="gcp-card" style={{ flexShrink: 0, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderBottom: "1px solid var(--proof-grey)" }}>
            ...
            />
            {query && (
              <button onClick={() => { setQuery(""); setActiveFilter("all"); }} style={{ border: "none", background: "transparent", cursor: "pointer", color: "var(--proof-text-secondary)" }}>
                <X size={15} />
              </button>
            )}
          </div>
          ...
          {showResults && rawResults.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", borderBottom: "1px solid var(--proof-grey)", background: "var(--proof-grey-bg)" }}>
              {(["all", "tests", "runs", "compare"] as const).map(f => {
                ...
                return (
                  <button key={f} onClick={() => setActiveFilter(f)}
                    style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 4, fontSize: 12, fontWeight: 500, border: isActive ? "none" : "1px solid var(--proof-grey)", background: isActive ? "var(--proof-blue)" : "var(--proof-surface)", color: isActive ? "white" : "var(--proof-text-secondary)", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "8px 16px", background: "var(--proof-grey-bg)", flexWrap: "wrap" }}>
            {[
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, color: "var(--proof-text-secondary)" }}>
                <kbd style={{ fontFamily: "var(--font-mono)", background: "var(--proof-surface)", border: "1px solid var(--proof-grey)", borderRadius: 4, padding: "2px 6px", display: "flex", alignItems: "center" }}>{k.icon}</kbd>
                {k.label}
              </div>
            ))}
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {hasResults && (
            <div className="gcp-card" style={{ overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid var(--proof-grey)", background: "var(--proof-grey-bg)" }}>
                <h2 style={{ fontSize: 13, fontWeight: 500 }}>Results for <span style={{ fontFamily: "var(--font-mono)", color: "var(--proof-blue)" }}>"{query}"</span></h2>
                <span style={{ fontSize: 12, color: "var(--proof-text-secondary)" }}>{filteredActive.length} results</span>
              </div>
              {filteredActive.map((r, i) => (
                <div key={`${r.kind}-${i}`}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", cursor: "pointer", borderBottom: i < filteredActive.length - 1 ? "1px solid var(--proof-grey)" : undefined, background: i === activeIdx ? "var(--proof-blue-bg)" : undefined, transition: "background 0.15s" }}
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--proof-text-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.sub}</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                    {r.meta && <span style={{ fontSize: 11, color: "var(--proof-text-secondary)", fontFamily: "var(--font-mono)" }}>{r.meta}</span>}
                    {i === activeIdx && <ChevronRight size={14} style={{ color: "var(--proof-blue)", flexShrink: 0 }} />}
                  </div>
                </div>
              ))}
            </div>
          ...
          {hasNoResults && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: "80px 0", color: "var(--proof-text-secondary)" }}>
            ...
            <div className="gcp-card" style={{ overflow: "hidden" }}>
              <div style={{ padding: 16 }}>
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 500, color: "var(--proof-text-secondary)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {RUNS.slice(-5).reverse().map((r, i) => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11, color: "var(--proof-text-secondary)" }}>{r.env} · {r.status}</span>
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    {QUICK_ACTIONS.map((a, i) => (
                      <div key={i} onClick={() => { if (i === 0) navigate("/start"); else if (i === 1) navigate("/compare"); else if (i === 2) navigate("/testdoc"); else navigate("/"); }}
                        style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 4, border: "1px solid var(--proof-grey)", cursor: "pointer", transition: "background 0.15s" }}
                        onMouseEnter={e => e.currentTarget.style.background = "var(--proof-surface-hover)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                      >
                        <a.icon size={14} style={{ color: a.color }} />
                        <span style={{ fontSize: 12, flex: 1 }}>{a.label}</span>
                        <kbd style={{ fontFamily: "var(--font-mono)", fontSize: 10, background: "var(--proof-grey-bg)", border: "1px solid var(--proof-grey)", borderRadius: 4, padding: "1px 4px", color: "var(--proof-text-secondary)" }}>⌘{a.shortcut}</kbd>
                      </div>
                    ))}
                  </div>
                </div>

...
import {
  Github, Check, Activity, LayoutDashboard, List, GitCompare,
...
const WORKFLOW_RUNS = [
  { id: "892", name: "Regression Suite", status: "running", duration: "3m 12s", branch: "main", actor: "devops-bot" },
  { id: "891", name: "Geo Gating Tests", status: "queued", duration: "—", branch: "feature/cache-opt", actor: "engineer-1" },
  { id: "890", name: "Smoke Tests", status: "completed", duration: "1m 45s", branch: "main", actor: "devops-bot" },
  { id: "889", name: "Regression Suite", status: "completed", duration: "2m 38s", branch: "staging", actor: "engineer-2" },
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {STAGES.map((stage, idx) => {
              ...
              return (
                <React.Fragment key={stage.id}>
                  <div className="pipeline-stage-enter" onClick={() => setActiveStage(idx)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer", minWidth: 100 }}>
                    <div className={`${isCompleted ? "stage-glass-completed" : isActive ? "stage-glass-active" : "stage-glass"}`} style={{
                      width: 72, height: 72, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4,
                      transition: "all 0.4s", transform: isActive ? "scale(1.08)" : "scale(1)",
                  ...
                  {idx < STAGES.length - 1 && (
                    <div className={isCompleted ? "connector-gradient-completed" : "connector-gradient"} style={{ flex: 1, height: 3, minWidth: 20, borderRadius: 2, margin: "0 4px", marginBottom: 24, opacity: isCompleted ? 1 : 0.5 }} />
                  )}
                </React.Fragment>
              );
            })}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  ...
                  <tr key={run.id}>
                    ...
                    <td>
                      <span className={`gcp-badge ${
                        ...
                        run.status === "failed" ? "gcp-badge-fail" :
                        run.status === "running" ? "gcp-badge-running" :
                        "gcp-badge-skip"
                      }`}>{run.status}</span>
                    </td>
          ...
          <div className="gcp-card" style={{ overflow: "hidden", display: "flex", flexDirection: "column" }}>
            ...
            <pre style={{ flex: 1, overflowY: "auto", margin: 0, padding: 14, background: "#1e1e1e", color: "#d4d4d4", fontFamily: "var(--font-mono)", fontSize: 11, lineHeight: 1.65, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
              {YAML_SPEC}
            </pre>
          </div>
        </div>
        ...
        <div className="gcp-card" style={{ padding: "14px 18px" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {[
              ...
              { href: "/compare", label: "Compare", icon: GitCompare, desc: "Baseline vs candidate diff" },

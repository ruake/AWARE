  ...
  const filtered = useMemo(() => {
    return runs
      ...
      .filter(
        (r) =>
          !query ||
          [r.id, r.label, r.suiteId, r.build, r.rev].some((v) =>
            v?.toLowerCase().includes(query.toLowerCase()),
          ),
      )
      .filter(
        (r) => !colLabelFilter || r.label.toLowerCase().includes(colLabelFilter.toLowerCase()),
      )
      .filter((r) => !colEnvFilter || r.env.toLowerCase().includes(colEnvFilter.toLowerCase()))
      .filter(
        (r) => !colSuiteFilter || r.suiteId.toLowerCase().includes(colSuiteFilter.toLowerCase()),
      );
    ...
    <PageWrapper className="proof-page space-y-5 relative">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gcp-text-muted mt-0.5">
            Test execution history across environments
          </p>
        </div>
        <motion.span
          variants={scaleIn}
      ...
      >
        ...
        <div className="flex items-center gap-1.5 p-1 rounded-lg bg-gcp-elevated/40 backdrop-blur-sm border border-gcp-border-soft/50">
          {(["ALL", "QA", "UAT", "PROD"] as const).map((e) => (
            <button
              key={e}
              onClick={() => startTransition(() => setEnvFilter(e as Env | "ALL"))}
        ...
        <div className="flex items-center gap-1.5 p-1 rounded-lg bg-gcp-elevated/40 backdrop-blur-sm border border-gcp-border-soft/50">
          {(["ALL", "PASS", "FAIL", "PARTIAL"] as const).map((s) => (
            <button
              key={s}
              onClick={() => startTransition(() => setStatusFilter(s as RunStatus | "ALL"))}
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all duration-200 border ${
                statusFilter === s
                  ? "bg-gcp-blue/20 text-gcp-blue-light border-gcp-blue/40 shadow-[0_0_8px_rgba(66,133,244,0.22)]"
                  : "bg-transparent text-gcp-text-secondary border-transparent hover:text-gcp-text hover:bg-gcp-elevated/60"
      ...
      >
        <span className="text-gcp-text-secondary font-medium tabular-nums">
          {deferredSorted.length} of {runs.length} runs
        </span>
        <span className="text-gcp-text-muted/40">|</span>
        <span className="text-gcp-text-muted">
          QA: <span className="text-gcp-text-secondary font-mono tabular-nums">{counts.QA}</span>
        </span>
        <span className="text-gcp-text-muted">
          UAT: <span className="text-gcp-text-secondary font-mono tabular-nums">{counts.UAT}</span>
        </span>
        <span className="text-gcp-text-muted">
          PROD:{" "}
          <span className="text-gcp-text-secondary font-mono tabular-nums">{counts.PROD}</span>
        </span>
      </motion.div>
      ...
      >
          ...
          <thead>
            <tr className="bg-gradient-to-r from-gcp-blue-bg/80 via-gcp-surface to-gcp-surface border-b border-gcp-border">
              <SortHeader
                ...
                currentSort={sort}
                onToggle={toggle}
                filterValue={colLabelFilter}
                onFilterChange={setColLabelFilter}
              />
              <SortHeader
                label="Env"
                sortKey="env"
                currentSort={sort}
                onToggle={toggle}
                filterValue={colEnvFilter}
                onFilterChange={setColEnvFilter}
                filterPlaceholder="Filter env…"
              />
              <SortHeader
                label="Suite"
                sortKey="suite"
                currentSort={sort}
                onToggle={toggle}
                filterValue={colSuiteFilter}
                onFilterChange={setColSuiteFilter}
              />
              <SortHeader
                label="Pass Rate"
                sortKey="passPct"
        ...
        {deferredSorted.length === 0 && (
          ...
          >
            ...
            <div className="text-gcp-text-muted text-sm font-medium">
              No runs match the current filters
            </div>
            <p className="text-gcp-text-muted/50 text-xs">

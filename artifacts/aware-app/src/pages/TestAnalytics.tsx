...
const PassRateMiniChart = memo(function PassRateMiniChart({
  data,
}: {
...
const FailureMiniChart = memo(function FailureMiniChart({
  data,
}: {
  data: { label: string; passRate: number }[];
}) {
...
const VelocityMiniChart = memo(function VelocityMiniChart({
  data,
}: {
...
const RunFrequencyChart = memo(function RunFrequencyChart({
  data,
}: {
  data: { name: string; count: number }[];
}) {
    ...
    <div style={{ height: 240, width: "100%" }}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data}>
          ...
          />
          <YAxis stroke="var(--proof-text-muted)" fontSize={10} tickLine={false} axisLine={false} />
          <Tooltip
            contentStyle={{
...
export default function TestAnalytics() {
  ...
  const categoryData = useMemo(
    () =>
      filteredRuns.map((r) => ({
        env: r.env,
        status: r.status === "FAIL" ? ("FAIL" as const) : ("PASS" as const),
      })),
    [filteredRuns],
  );
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        >
          ...
          <button
            ...
            style={{ border: "1px solid var(--proof-border)", gap: 8 }}
          >
            <Download size={16} />
            Export Report
          </button>
          ...
          >
            ...
            >
              <div>
                ...
                <div
                  style={{ fontSize: 32, fontWeight: 700, marginTop: 4, color: "var(--proof-red)" }}
                  className="metric-number"
                >
                  {failureRate}%
                </div>
            ...
            >
              <div>
                ...
                >
                  {runVelocity}{" "}
                  <span
                    style={{ fontSize: 14, color: "var(--proof-text-secondary)", fontWeight: 500 }}
                  >
                    runs/day
                  </span>
                </div>
            ...
            <table className="proof-table" style={{ border: "none" }}>
              <thead>
                <tr style={{ background: "var(--proof-surface-2)" }}>
                  ...
                  <th style={{ fontFamily: "var(--font-mono)", fontSize: 10, textAlign: "right" }}>
                    TREND
                  </th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={4} style={{ padding: "64px 24px", textAlign: "center" }}>
                      ...
                      >
                        ...
                        <div>
                          <div
                            style={{ fontSize: 15, fontWeight: 600, color: "var(--proof-text)" }}
                          >

...
function ImportModal({ onClose, toast }: { onClose: () => void; toast: (m: string) => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "var(--proof-surface)", borderRadius: 10, width: "min(600px, 92vw)", padding: 24, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", display: "flex", flexDirection: "column", gap: 14 }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={onClose} style={{ border: "none", background: "none", cursor: "pointer", color: "var(--proof-text-secondary)" }}><X size={18} /></button>
        </div>
        <p style={{ fontSize: 12, color: "var(--proof-text-secondary)" }}>Paste JSON, YAML, or JUnit XML — format is auto-detected. Required: <code style={{ background: "var(--proof-grey-bg)", padding: "1px 5px", borderRadius: 3 }}>name</code>, <code style={{ background: "var(--proof-grey-bg)", padding: "1px 5px", borderRadius: 3 }}>category</code></p>
        ...
        {result && (
          <div style={{ padding: 12, background: "var(--proof-grey-bg)", borderRadius: 6, fontSize: 13 }}>
            <span style={{ color: "var(--proof-green)", fontWeight: 600 }}>✓ {result.tests.length} imported</span>
            {result.errors.length > 0 && <span style={{ color: "var(--proof-red)", marginLeft: 12 }}>{result.errors.length} issues</span>}
            <span style={{ color: "var(--proof-text-secondary)", marginLeft: 12, fontSize: 11 }}>Format: {result.format}</span>
          </div>
        )}
...
function BulkActionsBar({ selected, onClear, onDelete, onExport, onStatusChange, onPriorityChange, onAddToSuite, suites }: {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "var(--proof-blue-bg)", border: "1px solid var(--proof-blue)", borderRadius: 6, fontSize: 13, flexWrap: "wrap" }}>
      ...
      <button onClick={onDelete} style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 8px", color: "var(--proof-red)", background: "none", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600 }}><Trash2 size={12} /> Delete</button>
      ...
      {onPriorityChange && (<><span style={{ fontSize: 11, color: "var(--proof-text-secondary)" }}>Priority:</span>
      ...
      {onAddToSuite && suites && (
        <div style={{ position: "relative", display: "inline-block" }}>
          ...
          {showSuitePicker && (
            <div style={{ position: "absolute", top: "100%", left: 0, marginTop: 4, background: "var(--proof-surface)", border: "1px solid var(--proof-grey)", borderRadius: 6, boxShadow: "0 4px 12px rgba(0,0,0,0.15)", zIndex: 50, minWidth: 200, padding: 4 }}>
              {suites.map(s => (
                <div key={s.id} onClick={() => { onAddToSuite(s.id); setShowSuitePicker(false); }} style={{ padding: "6px 10px", cursor: "pointer", borderRadius: 4, fontSize: 12 }} onMouseEnter={e => e.currentTarget.style.background = "var(--proof-grey-bg)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  {s.name}
                </div>
              ))}
            </div>
      ...
      <button onClick={onClear} style={{ padding: 4, border: "none", background: "none", cursor: "pointer", color: "var(--proof-text-secondary)" }}><X size={14} /></button>
    </div>
  );
}
    ...
    <AppLayout activeHref="/tests">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, height: "calc(100vh - 100px)", maxWidth: 1600, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 800, color: "var(--proof-text)", marginBottom: 2 }}>Test Manager</h1>
            <p style={{ fontSize: 12, color: "var(--proof-text-secondary)" }}>{tcs.length} test cases across {suites.length} suites</p>
          </div>
        ...
        {(() => {
          ...
          return (
            <div style={{ display: "flex", gap: 12, marginBottom: 14, fontSize: 12, color: "var(--proof-text-secondary)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div style={{ position: "relative", flex: 1 }}>
            <Search size={14} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "var(--proof-text-secondary)", pointerEvents: "none" }} />
          ...
          <span style={{ fontSize: 12, color: "var(--proof-text-secondary)", whiteSpace: "nowrap" }}>{filtered.length} / {tcs.length} shown</span>
          {Object.values(colFilters).some(f => f.text || f.selected.length > 0) && (
            <button onClick={() => setColFilters({})} style={{ fontSize: 12, color: "var(--proof-red)", background: "none", border: "none", cursor: "pointer", whiteSpace: "nowrap" }}>Clear filters</button>
          )}
        </div>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              <table className="gcp-table" style={{ margin: 0 }}>
                <thead style={{ position: "sticky", top: 0, background: "var(--proof-surface)", zIndex: 10 }}>
                  <tr>
                    <th style={{ width: 36 }}>
                      <input type="checkbox" checked={selectedIds.size === filtered.length && filtered.length > 0} onChange={toggleAll} style={{ accentColor: "var(--proof-blue)" }} />
                    </th>
                ...
                <tbody>
                  {filtered.map(tc => {
                    ...
                    return (
                      <tr key={tc.id}
                        ...
                        style={{
                          cursor: "pointer",
                          background: isPanelOpen ? "var(--proof-blue-bg)" : isSelected ? "var(--proof-grey-bg)" : undefined,
                      ...
                      >
                        ...
                        <td>
                          <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--proof-blue)", fontWeight: 600, maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={tc.name}>{tc.name}</div>
                          <div style={{ fontSize: 10, color: "var(--proof-text-secondary)", fontFamily: "var(--font-mono)" }}>{tc.id} · v{tc.version}</div>
                        </td>
                        ...
                        <td><span style={{ fontSize: 11, padding: "2px 8px", background: "var(--proof-surface)", border: "1px solid var(--proof-grey)", borderRadius: 4 }}>{tc.category}</span></td>
                        <td>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                            {tc.tags.slice(0, 2).map(t => <TagBadge key={t} tagId={t} />)}
                            {tc.tags.length > 2 && <span style={{ fontSize: 10, color: "var(--proof-text-secondary)" }}>+{tc.tags.length - 2}</span>}
                          </div>
                        </td>
                        <td style={{ fontSize: 11, color: "var(--proof-text-secondary)" }}>{tc.owner.split("@")[0]}</td>
                        ...
                        <td onClick={e => e.stopPropagation()}>
                          <div style={{ display: "flex", justifyContent: "center", gap: 4 }}>
                            ...
                            <button onClick={() => { if (confirm(`Delete "${tc.name}"?`)) handleDelete([tc.id]); }} title="Delete" style={{ padding: 4, border: "none", background: "none", cursor: "pointer", color: "var(--proof-red)" }}><Trash2 size={13} /></button>
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  {filtered.length === 0 && (
                    <tr><td colSpan={9} style={{ textAlign: "center", padding: 40, color: "var(--proof-text-secondary)", fontSize: 13 }}>No test cases match your filters</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            <div style={{ padding: "8px 14px", borderTop: "1px solid var(--proof-grey)", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 11, color: "var(--proof-text-secondary)", flexShrink: 0 }}>
              <span>{filtered.length} of {tcs.length} test cases</span>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => handleExport("csv")} className="gcp-button gcp-button-xs" style={{ color: "var(--proof-blue)", background: "none", border: "none" }}>Export CSV</button>
                <button onClick={() => handleExport("junit_xml")} className="gcp-button gcp-button-xs" style={{ color: "var(--proof-blue)", background: "none", border: "none" }}>Export JUnit XML</button>
              </div>
            </div>
            </div>
          </PanelErrorBoundary>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
      <div className="gcp-card" style={{ width: 640, maxHeight: "80vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--proof-grey)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
          ...
          <button onClick={onClose} style={{ border: "none", background: "none", cursor: "pointer", fontSize: 18, color: "var(--proof-text-secondary)" }}>×</button>
        </div>
        <div style={{ padding: "12px 18px", borderBottom: "1px solid var(--proof-grey)", display: "flex", gap: 8, flexShrink: 0 }}>
          ...
          <span style={{ flex: 1 }} />
          <span style={{ fontSize: 10, color: "var(--proof-text-secondary)", alignSelf: "center" }}>Download full test registry</span>
        </div>
        <div style={{ flex: 1, overflow: "auto", padding: "8px 18px" }}>
          {changes.length === 0 ? (
            <div style={{ padding: "32px 0", textAlign: "center", fontSize: 13, color: "var(--proof-text-secondary)" }}>No changes recorded yet</div>
          ) : (
            changes.slice(0, 100).map((entry, i) => (
              <div key={`${entry.testId}-${entry.version}`} style={{ display: "flex", gap: 10, padding: "8px 0", borderBottom: i < Math.min(changes.length, 100) - 1 ? "1px solid var(--proof-grey)" : "none" }}>
                <div style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--proof-blue-bg)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: "var(--proof-blue)", fontSize: 11, fontWeight: 700 }}>
                  {entry.author[0]?.toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12, fontWeight: 600 }}>{entry.summary}</span>
                    <span style={{ fontSize: 10, color: "var(--proof-text-secondary)" }}>{new Date(entry.timestamp).toLocaleDateString()}</span>
                  </div>
                  <div style={{ fontSize: 11, color: "var(--proof-text-secondary)", marginTop: 2 }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--proof-blue)" }}>{entry.testId}</span>
                    {' · '}{entry.testName}
                    {' · by '}{entry.author}
                  </div>
                  {entry.changes.length > 0 && (
                    <div style={{ fontSize: 10, color: "var(--proof-text-secondary)", marginTop: 4, display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {entry.changes.map((c, j) => (
                        <span key={j} style={{ background: "var(--proof-grey-bg)", padding: "1px 6px", borderRadius: 3 }}>{c}</span>
                      ))}
                    </div>
                  )}
                </div>

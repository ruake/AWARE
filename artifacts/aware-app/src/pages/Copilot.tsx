        ...
        if (typeof window !== "undefined" && (window as any).ai?.languageModel) {
          ...
          if (caps.available !== "no") {
            const s = await (window as any).ai.languageModel.create({
              systemPrompt:
                "You are PROOF Copilot, an AI assistant for a CDN observability dashboard. Answer questions about test runs, deployment health, and CDN performance. Be concise and technical.",
            });
            setSession(s);
            setAiReady(true);
            setMessages([
              {
                role: "assistant",
                content:
                  "Hello! I'm PROOF Copilot, powered by Chrome's built-in AI. Ask me about test runs, CDN health, or deployment status.",
              },
            ]);
          }
        }
    ...
    return (
      <AppLayout activeHref="/copilot">
        ...
        >
          ...
          >
            PROOF Copilot requires Chrome 128+ with the built-in Gemini Nano LLM enabled. Open{" "}
            <code
              style={{

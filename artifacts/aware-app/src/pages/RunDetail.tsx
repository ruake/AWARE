    ...
    <div className="group border border-gcp-border/60 rounded-lg bg-gcp-bg/30 backdrop-blur-sm transition-all duration-200 hover:border-gcp-border">
      ...
      >
        <div className="flex items-center gap-2">
          <Icon
            size={14}
            className="text-gcp-text-muted group-hover:text-gcp-blue transition-colors"
          />
          <span>{title}</span>
          {count !== undefined && (
        ...
        >
          <ChevronDown
            size={14}
            className="text-gcp-text-muted group-hover:text-gcp-text transition-colors"
          />
        </motion.div>
      </button>
...
const EvidencePanel = React.memo(function EvidencePanel({ result }: { result: TestResult }) {
  ...
  const timingColor = (label: string) => {
    ...
    if (label.toLowerCase().includes("tcp") || label.toLowerCase().includes("connect"))
      return "from-gcp-blue/60 to-gcp-blue/30";
    if (label.toLowerCase().includes("tls") || label.toLowerCase().includes("ssl"))
      return "from-gcp-purple/60 to-gcp-blue/30";
    if (label.toLowerCase().includes("ttfb")) return "from-gcp-green/60 to-gcp-green/30";
    if (label.toLowerCase().includes("download") || label.toLowerCase().includes("transfer"))
      return "from-gcp-blue/60 to-gcp-cyan/30";
            ...
            result.evidence.assertions.map((a, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-sm font-medium ${a.pass ? "text-gcp-green-light" : "text-gcp-red-light"}`}
                  >
                    {a.label}
                  </div>
                  {!a.pass && (
                    <div className="mt-2 space-y-1">
                      {a.expected && (
                        <div className="text-xs text-gcp-text-secondary">
                          <span className="text-gcp-text-muted">Expected:</span>{" "}
                          <span className="font-mono bg-gcp-elevated/50 px-1.5 py-0.5 rounded text-gcp-text">
                            {a.expected}
                          </span>
                        </div>
                      )}
                      {a.actual && (
                        <div className="text-xs text-gcp-text-secondary">
                          <span className="text-gcp-text-muted">Actual:</span>{" "}
                          <span className="font-mono bg-gcp-elevated/50 px-1.5 py-0.5 rounded text-gcp-red-light">
                            {a.actual}
                          </span>
                        </div>
                      )}
          ...
          {result.evidence?.request && (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-gcp-text-secondary font-mono truncate">
              ...
              {Object.entries(requestHeaders).length > 0 && (
                <CollapsibleSection
                  ...
                  count={Object.entries(requestHeaders).length}
                >
                  <div className="space-y-1">
                    {Object.entries(requestHeaders).map(([k, v]) => (
                      <div
                        key={k}
                        className="text-xs text-gcp-text-secondary font-mono break-all hover:bg-gcp-elevated/40 px-1.5 py-0.5 rounded transition-colors border-l-2 border-transparent hover:border-gcp-blue/30"
                      >
            ...
            >
              <div className="mb-3">
                {result.evidence.response.status !== undefined && (
                  <span
                    className={`inline-flex items-center gap-2 px-2.5 py-1 rounded border text-xs font-semibold font-mono ${statusColor} shadow-sm`}
                  >
              ...
              {Object.entries(responseHeaders).length > 0 && (
                <div className="space-y-2">
                  <CollapsibleSection
                    ...
                    count={Object.entries(responseHeaders).length}
                  >
                    <div className="space-y-1">
                      {Object.entries(responseHeaders).map(([k, v]) => (
                        <div
                          key={k}
                          className="text-xs text-gcp-text-secondary font-mono break-all hover:bg-gcp-elevated/40 px-1.5 py-0.5 rounded transition-colors border-l-2 border-transparent hover:border-gcp-green/30"
                        >
                          <span className="text-gcp-text-muted">{k}:</span> {v}
                        </div>
                      ))}
                      ...
                      <div className="space-y-2">
                        {cookies.map((c, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              className="border border-gcp-border/50 rounded-lg bg-gcp-surface/40 p-2.5 hover:bg-gcp-surface/60 transition-colors"
                            >
                              <div className="text-xs font-mono text-gcp-text font-semibold break-all">
                                {nv}
                              </div>
                              {attrs.length > 0 && (
                                <div className="mt-1.5 space-y-0.5">
                                  {attrs.map((a, j) => (
                                    <div
                                      key={j}
                                      className="text-[10px] text-gcp-text-muted font-mono flex items-center gap-1"
                                    >
...
export default function RunDetail() {
  ...
  const filtered = useMemo(() => {
    return results.filter((r) => {
      ...
      const nameMatch =
        !colNameFilter || r.name.toLowerCase().includes(colNameFilter.toLowerCase());
      const catMatch =
        !colCatFilter || r.category.toLowerCase().includes(colCatFilter.toLowerCase());
      ...
      >
        <div className="text-gcp-text-secondary space-y-4">
          <div className="glass-panel rounded-lg p-8 text-center">
            ...
            <Link
              href="/runs"
              className="inline-flex items-center gap-1.5 text-sm text-gcp-blue hover:text-gcp-blue-light transition-colors"
            >
              <motion.span
                animate={{ x: [0, -4, 0] }}
  ...
  return (
    <PageWrapper className="flex flex-col min-h-screen bg-gcp-bg text-gcp-text">
      ...
      >
        <Link
          href="/runs"
          className="group text-xs text-gcp-text-secondary hover:text-gcp-text transition-colors flex items-center gap-1.5"
        >
          <motion.span
      ...
      >
        <span
          className={`px-3 py-1.5 rounded-md text-xs font-mono font-semibold ${envBadgeClass(run.env)} shadow-sm`}
        >
          {run.env}
        </span>
        ...
        {[
          ...
          {
            label: "Failures",
            value: String(run.failures),
            color: run.failures > 0 ? "text-gcp-red" : "text-gcp-text-secondary",
          },
          ...
          >
            <span className="text-[10px] uppercase tracking-wider text-gcp-text-muted">
              {chip.label}
            </span>
            <span
              className={`text-sm ${chip.mono ? "font-mono tabular-nums" : ""} ${(chip as any).color ?? "text-gcp-text"}`}
            >
              {chip.value}
            </span>
          </motion.div>
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative flex-1 max-w-sm">
            <Search
              size={13}
              className="absolute left-3 top-1/2 -translate-y-1/2 text-gcp-text-muted pointer-events-none"
            />
            <input
              type="text"
        ...
        >
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-gcp-elevated/40 border border-gcp-border/30 text-gcp-text-secondary">
            Showing{" "}
            <span className="font-mono tabular-nums text-gcp-text font-semibold">
              {paged.length}
            </span>{" "}
            of{" "}
            <span className="font-mono tabular-nums text-gcp-text font-semibold">
          ...
          {statusFilter === "ALL" && (
            <>
              ...
              <span className="inline-flex items-center gap-1">
                ...
                <span className="text-gcp-green-light">
              ...
              <span className="inline-flex items-center gap-1">
                ...
                <span className="text-gcp-red-light">
                  ...
                  <React.Fragment key={result.id}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-gcp-text group-hover:text-gcp-text transition-colors">
                            {result.name}
                          </span>
                          {(() => {
                      ...
                      <td className="px-4 py-3 text-xs text-gcp-text-secondary">
                        {result.category}
                      </td>
                      <td className="px-4 py-3">
                        <StatusBadge status={result.status} />
                      </td>
                      <td className="px-4 py-3 text-xs font-mono tabular-nums text-gcp-text-secondary">
                      ...
                      <td className="px-4 py-3 text-gcp-text-muted">
                        ...
                        >
                          <ChevronDown
                            size={14}
                            className="group-hover:text-gcp-text transition-colors"
                          />
                        </motion.div>
                      </td>
                    </motion.tr>
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-16 text-center">
                    ...
                    >
                      ...
                      <div className="text-gcp-text-muted text-sm">

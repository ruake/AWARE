...
export default function AgentTrace({ toolCalls, providerType, streaming }: Props) {
  ...
  const totalMs = toolCalls.reduce(
    (acc, t) => (t.completedAt ? acc + (t.completedAt - t.startedAt) : acc),
    0,
  );
  const { label, color } = PROVIDER_LABEL[providerType] ?? {
    label: providerType,
      ...
      >
        ...
        {total > 0 && (
          <span style={{ color: "var(--proof-text-muted)", fontSize: 10.5 }}>
            ·{" "}
            <span style={{ color: "var(--proof-text-secondary)", fontWeight: 600 }}>
              {done}/{total}
            </span>{" "}
            tool{total !== 1 ? "s" : ""}
          ...
          {toolCalls.map((tc) => (
            ...
            >
              ...
              {Object.keys(tc.args).length > 0 && (
                ...
                >
                  ({JSON.stringify(tc.args).slice(0, 36)}
                  {JSON.stringify(tc.args).length > 36 ? "…" : ""})
                </span>
              )}
              {tc.completedAt && (
                <span
                  style={{ color: "var(--proof-text-muted)", fontSize: 9.5, marginLeft: "auto" }}
                >

...
function NodePill({ def, state, isLast }: {
  ...
  const borderColor = isRunning ? def.color
    : isDone  ? "rgba(52,211,153,0.35)"
    : isError ? "rgba(239,68,68,0.35)"
    : "rgba(255,255,255,0.06)";

  const bg = isRunning ? `${def.color}18`
    : isDone  ? "rgba(52,211,153,0.06)"
    : "rgba(255,255,255,0.03)";

  const labelColor = isRunning ? def.color
    : isDone  ? "var(--proof-text)"
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 0 }}>
      ...
      <div style={{
        ...
        {/* Status icon */}
        {isRunning  && <Loader2 size={10} style={{ color: def.color, animation: "spin 0.8s linear infinite", flexShrink: 0 }} />}
        {isDone     && <CheckCircle2 size={10} style={{ color: "#34d399", flexShrink: 0 }} />}
        {isError    && <XCircle size={10} style={{ color: "#f87171", flexShrink: 0 }} />}
        {isPending  && <Icon size={10} style={{ color: "var(--proof-text-muted)", flexShrink: 0, opacity: 0.4 }} />}

        {/* Label */}
        <span style={{
          fontSize: 10.5, fontWeight: isRunning || isDone ? 600 : 400,
        ...
        {isDone && state?.toolNames && state.toolNames.length > 0 && (
          <span style={{ fontSize: 9, color: "var(--proof-text-muted)", fontFamily: "var(--font-mono)", maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
...
export default function LangGraphPanel({ nodes, streaming, providerType }: Props) {
  const provider = PROVIDER_META[providerType] ?? { label: providerType, color: "var(--proof-blue)" };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ width: 5, height: 5, borderRadius: "50%", background: provider.color, animation: streaming ? "pulse 1.5s ease-in-out infinite" : "none" }} />
          {provider.label}
        </div>

        {/* LangGraph label */}
        <span style={{ fontSize: 9.5, color: "var(--proof-text-muted)", fontFamily: "var(--font-mono)" }}>
          LangGraph
        </span>

        {/* Node pipeline */}
        <div style={{ display: "flex", alignItems: "center", gap: 0, marginLeft: "auto", flexWrap: "nowrap", overflow: "hidden" }}>
          {NODE_DEFS.map((def, i) => (
            <NodePill
              key={def.id}
              def={def}
        ...
        {isComplete ? (
          <span style={{ fontSize: 9.5, color: "#34d399", fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>
            ✓ done
          </span>
        ) : streaming ? (
      ...
      {!isComplete && nodes.length > 0 && (() => {
        ...
        return (
          <div style={{ padding: "4px 12px 6px", display: "flex", alignItems: "center", gap: 6 }}>
            <Loader2 size={9} style={{ color: def?.color ?? "var(--proof-blue)", animation: "spin 0.8s linear infinite", flexShrink: 0 }} />

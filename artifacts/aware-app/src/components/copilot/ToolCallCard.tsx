...
import { CheckCircle2, XCircle, Loader2, ChevronDown, ChevronRight, Database, Clock } from "lucide-react";
...
const TOOL_META: Record<string, { label: string; verb: string; color: string; icon: string }> = {
  ...
  get_flaky_tests:      { label: "Flaky Tests",       verb: "Analyzing flakiness…",        color: "#f59e0b", icon: "⚡" },
  compare_environments: { label: "Env Comparison",    verb: "Comparing environments…",     color: "#8b5cf6", icon: "🔀" },
  get_promotion_status: { label: "Promotion Gate",    verb: "Checking promotion gate…",    color: "#10b981", icon: "🛡️" },
  get_failure_breakdown:{ label: "Failure Breakdown", verb: "Breaking down failures…",     color: "#ef4444", icon: "🔍" },
};

export default function ToolCallCard({ toolCall }: Props) {
  const [expanded, setExpanded] = React.useState(false);
  const meta = TOOL_META[toolCall.name] ?? { label: toolCall.name, verb: `Running ${toolCall.name}…`, color: "var(--proof-blue)", icon: "⚙️" };
    ...
    <div className="copilot-tool-card">
      ...
      >
        ...
        <div style={{
          ...
          background: isRunning ? "rgba(59,130,246,0.1)" : isDone ? "rgba(52,211,153,0.1)" : "rgba(248,113,113,0.1)",
          border: `1px solid ${isRunning ? "rgba(59,130,246,0.2)" : isDone ? "rgba(52,211,153,0.2)" : "rgba(248,113,113,0.2)"}`,
          display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
        }}>
          {isRunning && <Loader2 size={11} style={{ color: "#60a5fa", animation: "spin 1s linear infinite" }} />}
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: isRunning ? "#60a5fa" : isDone ? "var(--proof-text)" : "#f87171", flex: 1 }}>
          {isRunning ? meta.verb : meta.label}
        </span>

        {/* Duration */}
        {durationMs !== null && (
          <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 10, color: "var(--proof-text-muted)" }}>
            <Clock size={9} />
      ...
      {isDone && expanded && !!toolCall.result?.data && (
        <div style={{ padding: "0 12px 12px", borderTop: "1px solid var(--proof-border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 0 6px" }}>
            ...
            <span style={{ fontSize: 9.5, fontWeight: 600, color: "var(--proof-text-muted)", textTransform: "uppercase", letterSpacing: "0.5px" }}>Raw Data</span>
          </div>
          <pre style={{
            fontSize: 10.5, color: "var(--proof-text-secondary)",
            overflowX: "auto", maxHeight: 180, overflowY: "auto",
            margin: 0, fontFamily: "var(--font-mono)",
            background: "rgba(0,0,0,0.3)", borderRadius: 8,
            padding: "8px 10px", border: "1px solid var(--proof-border)",
            lineHeight: 1.5,
          }}>

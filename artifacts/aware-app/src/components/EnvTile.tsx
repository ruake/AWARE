...
const ENV_CONFIG = {
  QA: {
    ...
    accent: "border-t-gcp-yellow/60",
    iconBg: "bg-gcp-yellow/10",
    iconColor: "text-gcp-yellow",
  },
  UAT: {
    icon: TestTube2,
    label: "User Acceptance",
    accent: "border-t-gcp-blue/60",
    iconBg: "bg-gcp-blue/10",
    iconColor: "text-gcp-blue",
  },
  PROD: {
    icon: Server,
    label: "Production",
    accent: "border-t-gcp-green/60",
    iconBg: "bg-gcp-green/10",
    iconColor: "text-gcp-green",
  },
};
    ...
    >
      ...
      >
        <div className="mb-4 flex items-start justify-between">
          <div>
            <div className="mb-1.5 text-[11px] font-semibold uppercase tracking-widest text-gcp-text-muted">
              {cfg.label}
            </div>
            <div className="text-3xl font-bold tabular-nums tracking-tight">
              <span
                className={passColor}
                style={avgPass !== null ? { textShadow: `0 0 24px currentColor` } : undefined}
              >
                {avgPass !== null ? `${avgPass}%` : "N/A"}
              </span>
            </div>
          </div>
          <div
            className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl border border-gcp-border-soft ${cfg.iconBg} transition-all duration-200 group-hover:scale-110 group-hover:shadow-lg`}
          >
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-gcp-text-muted">
            ...
            {latest && (
              <span className="ml-2 text-gcp-text-muted">· {relativeTime(latest.started)}</span>
            )}
          </div>

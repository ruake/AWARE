...
export function CTAStatCard({ label, value, subtitle, onClick, active, accentColor = "var(--proof-blue)", icon }: any) {
  return (
    <div onClick={onClick} className={`glass-panel cursor-pointer flex flex-col gap-2 rounded-xl border border-[var(--proof-border)] p-4 transition-all hover:-translate-y-1 ${active ? "ring-1" : ""}`} style={{ borderColor: active ? accentColor : undefined, ringColor: active ? accentColor : undefined }}>

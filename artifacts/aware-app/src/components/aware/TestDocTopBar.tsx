  ...
  return (
    <div className="gcp-card" style={{ padding: 16, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, position: "sticky", top: 0, zIndex: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <button onClick={() => window.history.back()} style={{ color: "var(--proof-text-secondary)", background: "none", border: "none", cursor: "pointer", padding: 4, borderRadius: "50%", display: "flex", alignItems: "center", gap: 4 }}>
          <ArrowLeft size={18} />
          <span style={{ fontSize: 13 }}>Back</span>
        </button>
        <div style={{ width: 1, height: 24, background: "var(--proof-grey)" }} />
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <span style={{ padding: "4px 8px", background: "var(--proof-grey-bg)", fontSize: 12, fontWeight: 500, borderRadius: 4, color: "var(--proof-text-secondary)", border: "1px solid var(--proof-grey)" }}>{testCategory}</span>
          <span style={{ padding: "4px 8px", background: "var(--proof-grey-bg)", fontSize: 12, fontWeight: 500, borderRadius: 4, color: "var(--proof-text-secondary)", border: "1px solid var(--proof-grey)" }}>{testSuite}</span>
        </div>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <button style={{ padding: 8, border: "none", background: "none", cursor: "pointer", color: "var(--proof-text-secondary)", borderRadius: "50%" }} title="Pin Test">
          <Pin size={18} />
        </button>

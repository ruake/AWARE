...
export function ConsoleSidebar({ navGroups, activePath, collapsed, onToggleCollapse }: ConsoleSidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "8px 0", display: "flex", flexDirection: "column", gap: 2 }}>
        {navGroups.map((group, gi) => (
          <div key={`${group.title}-${gi}`}>
            {gi > 0 && (
              <div style={{ margin: "6px 10px", borderTop: "1px solid var(--proof-border)", opacity: 0.5 }} />
            )}
              ...
              return (
                ...
                >
                  {IconComp ? (
                    ...
                    <div style={{ width: 16, height: 16, borderRadius: 4, background: "var(--proof-border)", flexShrink: 0 }} />
                  )}
                  <span style={{
                    fontSize: 12.5,
      ...
      {/* Collapse toggle */}
      <div style={{ borderTop: "1px solid var(--proof-border)", padding: "4px 6px", flexShrink: 0 }}>
        <button
          ...
          style={{
            ...
            color: "var(--proof-text-muted)", borderRadius: 6,
            transition: "background 0.15s, color 0.15s",
            fontSize: 11, fontFamily: "var(--font-sans)",
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.05)"; (e.currentTarget as HTMLElement).style.color = "var(--proof-text)"; }}
        ...
        >
          {!expanded ? (
            ...
            <>
              <ChevronLeft size={15} />
              <span style={{ opacity: expanded ? 1 : 0, transition: "opacity 0.15s" }}>Collapse</span>
            </>
          )}
        </button>
      </div>

...
function FilterSection({
  envIds, suiteIds, onEnvChange, onSuiteChange,
}: {
  envIds: string[];
  suiteIds: string[];
...
function RunsStatSummary({ envFilteredRuns }: { envFilteredRuns: Run[] }) {
  ...
  const avgDurationStr = avgMs < 60000 ? `${(avgMs / 1000).toFixed(1)}s` : `${Math.round(avgMs / 60000)}m`;

  const items = [
    { label: "Total", value: total.toString(), color: "var(--proof-blue)", onClick: () => navigate("/runs") },
...
function RunsListBanner({ running }: { running: Run[] }) {
  return running.length > 0 ? (
    <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ padding: "8px 10px", borderRadius: 6, background: "linear-gradient(90deg, rgba(59,130,246,0.1) 0%, rgba(91,138,245,0.06) 100%)", border: "1px solid rgba(59,130,246,0.25)", borderLeft: "3px solid var(--proof-blue-hover)", display: "flex", alignItems: "center", gap: 8 }}>
        <Loader2 size={12} style={{ color: "var(--proof-blue-hover)", animation: "spin 1s linear infinite", flexShrink: 0 }} />
        <span style={{ flex: 1, fontSize: 11, fontWeight: 600, color: "var(--proof-text)" }}>{running.length} running</span>
        <a href={GH_ACTIONS_URL} target="_blank" rel="noopener noreferrer" style={{ fontSize: 9, fontWeight: 600, color: "var(--proof-blue-hover)", textDecoration: "none", display: "flex", alignItems: "center", gap: 3 }}>
    ...
    <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ padding: "8px 10px", borderRadius: 6, background: "linear-gradient(90deg, rgba(34,197,94,0.08) 0%, rgba(34,197,94,0.03) 100%)", border: "1px solid rgba(34,197,94,0.18)", borderLeft: "3px solid #22c55e", display: "flex", alignItems: "center", gap: 8 }}>
        <Check size={12} style={{ color: "var(--proof-green)", flexShrink: 0 }} />
        <span style={{ flex: 1, fontSize: 11, fontWeight: 600, color: "var(--proof-text)" }}>All idle</span>
        <a href={GH_ACTIONS_URL} target="_blank" rel="noopener noreferrer" style={{ fontSize: 9, fontWeight: 600, color: "var(--proof-green)", textDecoration: "none", display: "flex", alignItems: "center", gap: 3 }}>
  ...
  return (
    <div style={{ padding: "6px 10px", borderBottom: "1px solid var(--proof-border)", display: "flex", gap: 4, flexWrap: "wrap" }}>
      {items.map(({ href, icon: Ic, label }) => (
        <button key={href} onClick={() => navigate(href)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 4, border: "1px solid var(--proof-border-strong)", background: "var(--proof-surface-2)", color: "var(--proof-text-secondary)", cursor: "pointer", fontSize: 10, fontWeight: 500, transition: "all 0.12s" }}
          onMouseEnter={(e) => { e.currentTarget.style.color = "var(--proof-text)"; e.currentTarget.style.borderColor = "rgba(91,138,245,0.35)"; }}
          onMouseLeave={(e) => { e.currentTarget.style.color = "var(--proof-text-secondary)"; e.currentTarget.style.borderColor = "var(--proof-border-strong)"; }}
        >
          <Ic size={11} /> {label}
        </button>
      ))}
...
function RunsListFilters() {
  ...
  const selectedEnvIds = useSyncExternalStore(subscribeToSelectedEnv, getSelectedEnvSnapshot).envIds;
  const base = selectedEnvIds.length > 0 ? allRuns.filter((r) => selectedEnvIds.includes(r.envId)) : allRuns;
  const filtered = base.filter((r) => {
    if (statusFilter !== "all" && r.status !== statusFilter) return false;
    if (suiteFilter !== "all" && r.suiteId !== suiteFilter) return false;
    if (envFilter !== "all" && r.env !== envFilter) return false;
    if (search) { const q = search.toLowerCase(); if (!r.id.toLowerCase().includes(q) && !r.env.toLowerCase().includes(q) && !r.suiteId.toLowerCase().includes(q)) return false; }
    return true;
  });
  ...
  return (
    <div style={{ padding: "6px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4, border: "1px solid var(--proof-border)", borderRadius: 4, padding: "3px 6px", marginBottom: 6 }}>
        <Search size={11} style={{ color: "var(--proof-text-muted)", flexShrink: 0 }} />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search runs..." style={{ border: "none", outline: "none", fontSize: 11, background: "transparent", flex: 1, minWidth: 0, color: "var(--proof-text)" }} />
      </div>
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ fontSize: 10, padding: "2px 5px", border: "1px solid var(--proof-border)", borderRadius: 3, background: "var(--proof-surface)", color: "var(--proof-text)", flex: 1, minWidth: 0 }}>
        ...
        <select value={suiteFilter} onChange={(e) => setSuiteFilter(e.target.value)} style={{ fontSize: 10, padding: "2px 5px", border: "1px solid var(--proof-border)", borderRadius: 3, background: "var(--proof-surface)", color: "var(--proof-text)", flex: 1, minWidth: 0 }}>
          <option value="all">All suites</option>
          {suites.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={envFilter} onChange={(e) => setEnvFilter(e.target.value)} style={{ fontSize: 10, padding: "2px 5px", border: "1px solid var(--proof-border)", borderRadius: 3, background: "var(--proof-surface)", color: "var(--proof-text)", flex: 1, minWidth: 0 }}>
          <option value="all">All envs</option>
          {envs.map((e) => <option key={e} value={e}>{e}</option>)}
        </select>
        {hasActiveFilters && (
          <button onClick={clearFilters} style={{ fontSize: 10, color: "var(--proof-red-bright)", background: "none", border: "none", cursor: "pointer", padding: "2px 4px", display: "flex", alignItems: "center", gap: 2 }}>
            <X size={10} /> Clear
          </button>
        )}
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4 }}>
        <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
          ...
          {suiteFilter !== "all" && <FilterBadge label={`suite=${suiteFilter}`} onRemove={() => setSuiteFilter("all")} />}
          {envFilter !== "all" && <FilterBadge label={`env=${envFilter}`} onRemove={() => setEnvFilter("all")} />}
          {search && <FilterBadge label={`"${search}"`} onRemove={() => setSearch("")} />}
        </div>
        <span style={{ fontSize: 10, color: "var(--proof-text-muted)", fontFamily: "var(--font-mono)" }}>{filtered.length}/{base.length}</span>
      </div>
    </div>
  );
}
...
function FilterBadge({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 2, fontSize: 9, padding: "1px 5px", borderRadius: 3, background: "var(--proof-hover)", border: "1px solid var(--proof-border)", color: "var(--proof-text-secondary)", cursor: "pointer" }} onClick={onRemove}>
      {label} <X size={8} />
    </span>
  );
}
...
function RunsGhActions() {
  return (
    <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 6, border: "1px solid var(--proof-grey)" }}>
        <Github size={16} style={{ color: "var(--proof-blue)", flexShrink: 0 }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "var(--proof-text)" }}>GitHub Actions</div>
          <div style={{ fontSize: 9, color: "var(--proof-text-secondary)", marginTop: 1 }}>Monitor live runs & review results</div>
        </div>
        <a href={GH_ACTIONS_URL} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 3, padding: "4px 8px", borderRadius: 4, border: "1px solid var(--proof-blue)", background: "var(--proof-blue-bg)", color: "var(--proof-blue)", cursor: "pointer", fontSize: 9, fontWeight: 600, textDecoration: "none", flexShrink: 0 }}>
  ...
  return (
    <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 6 }}>
        <button onClick={() => navigate("/runs")} style={{ border: "none", background: "transparent", cursor: "pointer", color: "var(--proof-text-secondary)", padding: 0, display: "flex", alignItems: "center", fontSize: 10, fontWeight: 600 }}>
          <ArrowLeft size={11} /> Runs
        </button>
        <span style={{ color: "var(--proof-border-strong)", fontSize: 11 }}>/</span>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, fontWeight: 600, color: "var(--proof-text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{run.id}</span>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
        <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 3, fontWeight: 600, background: run.status === "PASS" ? "rgba(34,197,94,0.12)" : run.status === "FAIL" ? "rgba(239,68,68,0.12)" : "rgba(234,179,8,0.12)", color: run.status === "PASS" ? "var(--proof-green)" : run.status === "FAIL" ? "var(--proof-red)" : "var(--proof-yellow)" }}>{run.status}</span>
        <span style={{ fontSize: 10, fontFamily: "var(--font-mono)", fontWeight: 700, color: passRate === 100 ? "var(--proof-green)" : passRate < 90 ? "var(--proof-red)" : "var(--proof-yellow)" }}>{passRate}%</span>
        <span style={{ fontSize: 9, color: "var(--proof-text-secondary)", fontFamily: "var(--font-mono)" }}>+{passCount}/-{failCount}</span>
        <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--proof-text-muted)" }}>{run.duration}</span>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6 }}>
        <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: "var(--proof-text-muted)", padding: "1px 5px", background: "rgba(255,255,255,0.04)", border: "1px solid var(--proof-border)", borderRadius: 3 }}>{run.env} · {run.envId} · build {run.build}</span>
      </div>
      <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
        <button onClick={() => { navigator.clipboard.writeText(window.location.href); }} style={{ border: "1px solid var(--proof-border)", background: "var(--proof-surface)", cursor: "pointer", padding: "2px 6px", borderRadius: 3, color: "var(--proof-text-secondary)", display: "flex", alignItems: "center", gap: 3, fontSize: 9 }}><Share2 size={9} /> Share</button>
        <button onClick={() => navigate(`/compare?candidate=${run.id}&baseline=${getRuns().at(-1)?.id}`)} style={{ border: "1px solid var(--proof-border)", background: "var(--proof-surface)", cursor: "pointer", padding: "2px 6px", borderRadius: 3, color: "var(--proof-text-secondary)", display: "flex", alignItems: "center", gap: 3, fontSize: 9 }}><GitCompare size={9} /> Diff</button>
        <a href={`https://github.com/ruake/AWARE/actions/runs/${run.id}`} target="_blank" rel="noopener" style={{ border: "1px solid var(--proof-border)", background: "var(--proof-surface)", cursor: "pointer", padding: "2px 6px", borderRadius: 3, color: "var(--proof-text-secondary)", display: "flex", alignItems: "center", textDecoration: "none", fontSize: 9 }}><Github size={9} /></a>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: "6px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: collapsed ? 0 : 6 }}>
        {!collapsed && <span style={{ fontSize: 9, fontWeight: 600, color: "var(--proof-text-secondary)", textTransform: "uppercase", letterSpacing: "0.4px" }}>Run KPIs</span>}
        <button onClick={() => setCollapsed((c) => !c)} style={{ border: "none", background: "transparent", cursor: "pointer", color: "var(--proof-text-secondary)", padding: 2, display: "flex", marginLeft: "auto" }}>
          <ChevronDown size={12} style={{ transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)", transition: "transform 0.15s" }} />
        </button>
      </div>
      {!collapsed && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {categories.length > 0 && (
            <div style={{ padding: "6px 8px", borderRadius: 4, border: "1px solid var(--proof-grey)", background: "var(--proof-grey-bg)" }}>
              <div style={{ fontSize: 8, color: "var(--proof-text-secondary)", textTransform: "uppercase", letterSpacing: "0.3px", marginBottom: 2 }}>By Category</div>
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 7, color: "var(--proof-text-muted)", fontFamily: "var(--font-mono)", minWidth: 16, textAlign: "right" }}>{pct}%</span>
                  </div>
                );
              })}
            </div>
...
function RunDetailFilters() {
  ...
  return (
    <div style={{ padding: "6px 10px", borderBottom: "1px solid var(--proof-border)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4, border: "1px solid var(--proof-border)", borderRadius: 4, padding: "3px 6px", marginBottom: 4 }}>
        <Search size={11} style={{ color: "var(--proof-text-muted)", flexShrink: 0 }} />
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tests..." style={{ border: "none", outline: "none", fontSize: 11, background: "transparent", flex: 1, minWidth: 0, color: "var(--proof-text)" }} />
      </div>
      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
        ...
        <select value={catFilter} onChange={(e) => setCatFilter(e.target.value)} style={{ fontSize: 10, padding: "2px 5px", border: "1px solid var(--proof-border)", borderRadius: 3, background: "var(--proof-surface)", color: "var(--proof-text)", flex: 1, minWidth: 0 }}>
          <option value="all">All cats</option>
          {categories.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <span style={{ fontSize: 9, color: "var(--proof-text-muted)", fontFamily: "var(--font-mono)", flexShrink: 0 }}>{filtered.length}/{results.length}</span>
      </div>
    </div>
  );
}
...
export function RunsPanel() {
  ...
  const allRuns = useSyncExternalStore(subscribeToRuns, getRuns);
  const envFilteredRuns = envSnap.envIds.length > 0 ? allRuns.filter((r) => envSnap.envIds.includes(r.envId)) : allRuns;
  const running = envFilteredRuns.filter((r) => r.status === "RUNNING");
  ...
  return (
    <>
      <FilterSection envIds={envSnap.envIds} suiteIds={[]} onEnvChange={() => {}} onSuiteChange={() => {}} />

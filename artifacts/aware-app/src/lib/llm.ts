...
import { nextTcId, testCasesStore, saveToStorage, _notify, LS_TC_KEY } from "./data";
...
What would you like me to help with?`;
  ...
  return {
    content,
    finishReason: "stop",
    usage: { promptTokens: messages.reduce((a, m) => a + m.content.length / 4, 0), completionTokens: content.length / 4, totalTokens: 0 },
  };
}
...
class WebLLMProvider implements ILLMProvider {
  ...
  private async _init() {
    try {
      ...
      this.engine = await CreateMLCEngine(this.config.model || "Llama-3.2-1B-Instruct-q4f32_1-MLC");
    } catch (e) {
      throw new Error(`WebLLM initialization failed: ${e}. Ensure @mlc-ai/web-llm is installed and WebGPU is available.`);
    }
  }
...
export async function generateTestsWithLLM(
  ...
  const skill = SKILLS.find(s => s.id === "generate-tests");
  ...
  try {
    ...
    for (const item of items.slice(0, params.count)) {
      const idx = nextTcId + idOffset;
      const tc: TestCase = {
        id: `tc_${idx}`,
        name: item.name ?? `Generated Test ${idx}`,
        ...
        owner: item.owner ?? params.category,
        suiteIds: [...params.suites],
        automated: true,
        scriptPath: item.scriptPath ?? `tests/generated/llm/tc_${idx}.spec.ts`,
        ...
        version: 1,
        changelog: [{ version: 1, timestamp: now, author: "llm", summary: "AI-generated", changes: ["Generated by LLM"] }],
        createdAt: now,
        updatedAt: now,
      };
      testCases.push(tc);
    ...
    throw new Error(`Failed to parse LLM response as test cases. Raw: ${res.content.substring(0, 200)}`);
  }

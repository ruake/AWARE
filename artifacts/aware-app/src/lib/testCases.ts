...
export function createTestCase(data: Omit<TestCase, "id" | "createdAt" | "updatedAt">): TestCase {
  ...
  if (!tc.documentation || tc.documentation.trim() === "") {
    tc.documentation = generateDocumentation(tc);
  }
...
export function exportTestCases(format: "json" | "csv"): string {
  ...
  return JSON.stringify(testCasesStore.map(tc => ({
    ...
    filmstrip: tc.filmstrip, predicates: tc.predicates, changelog: tc.changelog,
    preconditions: tc.preconditions, expectedBehavior: tc.expectedBehavior,
    documentation: tc.documentation, relatedTestIds: tc.relatedTestIds,
  })), null, 2);
}
...
export function exportTestsAsJunitXml(suiteId?: string): string {
  ...
  lines.push(`<testsuites name="PROOF Tests" tests="${tests.length}">`);
...
export function generateTestCases(params: GenerateParams): TestCase[] {
  ...
  for (let i = 0; i < params.count; i++) {
    ...
    const tc: TestCase = {
      ...
      relatedTestIds: [],
      requestHeaders: { "Accept": "application/json", "User-Agent": "PROOF-TestRunner/2.0" },
      cookies: {},
...
export function generateDocumentation(tc: TestCase): string {
  const lines = [
    `# ${tc.name}`,
    ``,
    `## Overview`,
    ...
    tc.requestHeaders && Object.keys(tc.requestHeaders).length > 0 ? `### Headers\n` + Object.entries(tc.requestHeaders).map(([k, v]) => `- \`${k}: ${v}\``).join("\n") : "No custom headers configured.",
    ``,
    tc.cookies && Object.keys(tc.cookies).length > 0 ? `### Cookies\n` + Object.entries(tc.cookies).map(([k, v]) => `- \`${k}=${v}\``).join("\n") : "No cookies configured.",
    ``,
    `## Predicates`,
    tc.predicates && tc.predicates.length > 0
      ? tc.predicates.map((p, i) => `${i + 1}. **${p.type}**: \`${p.field || ""}\` ${p.operator} \`${p.expected}\`${p.description ? ` — ${p.description}` : ""}`).join("\n")
      : "No predicates defined.",
    ``,
    `## Execution`,
    tc.automated ? `This test is automated and will be executed as part of the test suite.` : `This test requires manual execution.`,
    ``,
    `## Changelog`,
    tc.changelog && tc.changelog.length > 0
      ? tc.changelog.map(c => `- **${c.timestamp}** (${c.author}): ${c.summary}`).join("\n")
      : "No changes recorded.",
    ``,
    `*Documentation auto-generated by PROOF on ${new Date().toISOString().split("T")[0]}*`,
  ];
  return lines.join("\n");
}

...
export function computeAnomalyScores(): AnomalyScore[] {
  ...
  const stdPass = Math.sqrt(passPcts.reduce((s, v) => s + (v - meanPass) ** 2, 0) / passPcts.length) || 1;

  const meanDur = durations.reduce((a, b) => a + b, 0) / durations.length;
  const stdDur = Math.sqrt(durations.reduce((s, v) => s + (v - meanDur) ** 2, 0) / durations.length) || 1;
...
export function detectRunAnomalies(runs: Run[], threshold: number = 2): RunAnomaly[] {
  ...
  if (std === 0) return runs.map(r => ({ runId: r.id, passPct: r.passPct, zScore: 0, anomalous: false }));
  return runs.map(r => {

...
import { executeDataQuery } from "./dataQueries";
import { getSystemPromptForUseCase } from "./prompts";
import { getProvider } from "@/lib/llm";
import { RUNS, DIFF_ROWS, getTestResultsForRun, ENV_SUMMARY } from "@/lib/runs";
    ...
    try {
      const completion = await provider.complete({
        messages: [
          ...
          { role: "user", content: `Run ${request.useCaseId} analysis with these parameters: ${JSON.stringify(request.parameters)}` },
        ],
        temperature: 0.3,
        maxTokens: 2048,
      });
...
function fallbackFailureAnalysis(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const categoryBreakdown = Object.entries(byCategory)
    ...
    .map(([cat, s]) => `${cat}: ${s.failed}/${s.total} failed (${Math.round(s.failed / s.total * 100)}%)`);
  ...
  const failCatChart = JSON.stringify({
    ...
    rows: Object.entries(byCategory).filter(([, s]) => s.failed > 0).map(([cat, s]) => [cat, s.failed]),
    colors: ["#ef4444"],
    options: { legend: { position: "none" }, hAxis: { textStyle: { fontSize: 10 } }, vAxis: { textStyle: { fontSize: 9 } } },
  });
  const summary = failuresArr.length === 0
  ...
  return {
    ...
    confidence: 1,
    recommendations: failuresArr.length > 0
      ? [`Investigate ${byCategory[Object.keys(byCategory).find(k => byCategory[k].failed > 0) || ""]?.tests[0] || "failures"}`, categoryBreakdown.length > 0 ? `Focus on ${categoryBreakdown[0]?.split(":")[0] || "failing"} category` : "", failuresArr.length > 5 ? "Consider rolling back the last build change" : "Review individual failures for patterns"]
...
function fallbackFlakyDetection(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const recentRuns = [...RUNS].sort((a, b) => new Date(b.started).getTime() - new Date(a.started).getTime()).slice(0, lookback);
  ...
  for (const run of recentRuns) {
    ...
    for (const r of results) {
      ...
      if (!entry) { entry = { id: r.id, flips: 0, score: 0, sequence: "" }; flakyTests.push(entry); }
    }
  }
  ...
  const flaky = Object.entries(testHistory)
    ...
    .map(([id, statuses]) => {
      ...
      return { id, flips, score: Math.round(flips / (statuses.length - 1) * 100), sequence: statuses.join(" → ") };
    })
    .filter(f => f.score > 0)
  ...
  const flakyChart = JSON.stringify({
    ...
    colors: ["#f59e0b"],
    options: { legend: { position: "none" }, hAxis: { textStyle: { fontSize: 10 } }, vAxis: { textStyle: { fontSize: 9 }, minValue: 0, maxValue: 100 } },
  });
  const summary = flaky.length === 0
  ...
  return {
    ...
    confidence: 1,
    recommendations: flaky.filter(f => f.score > 20).map(f => `Quarantine ${f.id} (${f.score}% flakiness)`),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackAnomalyDetection(): AIAnalysisResult {
  ...
  if (sortedRuns.length >= 3) {
    ...
    for (const run of sortedRuns) {
      ...
      if (Math.abs(z) > 1.5) {
        anomalies.push({ runId: run.id, passPct: run.passPct, zScore: Math.round(z * 100) / 100, flag: z < 0 ? "below average" : "above average" });
      }
    }
  }
  const anomalyRunIds = [...new Set(anomalies.map(a => a.runId))];
  const passRateChart = JSON.stringify({
    ...
    curveType: "function",
    colors: ["#5b8af5"],
    options: { vAxis: { minValue: 0, maxValue: 100, textStyle: { fontSize: 10 } }, hAxis: { textStyle: { fontSize: 8 } }, legend: { position: "none" } },
  });
  ...
  return {
    ...
    confidence: 1,
    recommendations: anomalies.filter(a => a.flag === "below average").map(a => `Investigate run ${a.runId} (${a.passPct}% pass rate — significantly below mean)`),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackRiskScoring(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  return {
    ...
    confidence: 1,
    recommendations: level === "LOW" ? ["Safe to deploy", "Monitor next run"] : level === "MEDIUM" ? [`Investigate ${run.failures} failures before deploy`, "Check if failures are env-specific"] : ["BLOCKING: Do not deploy", `Critical: ${run.failures} failures at ${run.passPct}% pass rate`, "Roll back to previous build"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackEnvComparison(): AIAnalysisResult {
  ...
  const summary = envLabels.length >= 2
  ...
  return {
    ...
    data: envGroups,
    confidence: 1,
    recommendations: envLabels.length >= 2 && envGroups[envLabels[0]]?.avgPass !== envGroups[envLabels[1]]?.avgPass ? ["Investigate environment-specific failures", "Check configuration parity between environments"] : ["Environments are consistent", "Continue monitoring for drift"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackCategoryHealth(): AIAnalysisResult {
  ...
  const healthScores = Object.entries(allResults).map(([cat, s]) => ({
    category: cat,
  ...
  const summary = healthScores.length === 0
    ? "No category data available."
    : `Category health across ${healthScores.length} categories. ${unhealthy.length} unhealthy: ${unhealthy.map(h => `${h.category} (${h.passRate}%)`).join(", ")}. Healthiest: ${healthScores.filter(h => h.health === "healthy").slice(0, 2).map(h => `${h.category} (${h.passRate}%)`).join(", ")}.`;
  return {
    ...
    confidence: 1,
    recommendations: [...unhealthy.map(h => `Investigate ${h.category} (${h.passRate}% pass rate)`), ...(unhealthy.length > 0 ? ["Schedule focused review for unhealthy categories"] : ["All categories healthy — continue monitoring"])],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackSmartAlerting(): AIAnalysisResult {
  ...
  if (latest && latest.failures > 0) alerts.push({ severity: latest.failures > 10 ? "critical" : "warning", title: `${latest.failures} failures in latest run`, description: `Run ${latest.id} (${latest.build}) has ${latest.failures} failures at ${latest.passPct}% pass rate.` });
  if (latest && prev && latest.passPct < prev.passPct - 10) alerts.push({ severity: "critical", title: "Significant pass rate drop", description: `Pass rate dropped from ${prev.passPct}% to ${latest.passPct}% between builds ${prev.build} and ${latest.build}.` });
  if (latest && prev && latest.durationMs > prev.durationMs * 1.5) alerts.push({ severity: "warning", title: "Duration spike detected", description: `Run duration increased from ${prev.durationMs}ms to ${latest.durationMs}ms.` });
  const lowScoreRuns = sortedRuns.filter(r => r.passPct < 80);
  if (lowScoreRuns.length > 1) alerts.push({ severity: "warning", title: `${lowScoreRuns.length} runs below 80% pass rate`, description: `${lowScoreRuns.map(r => `Run ${r.id}: ${r.passPct}%`).join(", ")}` });
...
function fallbackSuiteHealth(): AIAnalysisResult {
  ...
  const summary = suiteHealth.length === 0
...
function fallbackDurationBudget(): AIAnalysisResult {
  ...
  const summary = slowTests.length === 0
...
function fallbackRedundancyDetection(): AIAnalysisResult {
  ...
  const summary = redundancies.length === 0
  ...
  return {
    ...
    confidence: 1,
    recommendations: redundancies.slice(0, 3).map(r => `Review ${r.tests.length} tests in ${r.category} for consolidation: ${r.tests.map(t => t.id).join(", ")}`),
    generatedAt: new Date().toISOString(),
  };
}

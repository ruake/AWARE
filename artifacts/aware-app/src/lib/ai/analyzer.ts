...
export async function runLangGraphChat(
  ...
  graph.addNode({
    ...
    execute: async (ctx) => {
      const context = buildAIContext();
      const systemPrompt = buildSystemPrompt(context);
      logInfo("context_build", "Context built", `${context.stats.totalRuns} runs, ${context.stats.totalTests} tests`);
  ...
  graph.addNode({
    ...
    execute: async (ctx) => {
      ...
      logInfo("llm_chat", `Sending ${chatMessages.length} messages to ${config.model || config.provider}`);
      ...
      if (truncated.length < chatMessages.length) {
        logInfo("llm_chat", `Truncated ${chatMessages.length - truncated.length} messages to fit context window`);
      }
      ...
      if (completion.finishReason === "error") {
        const isContextError = /context window|context_window|prompt tokens exceed/i.test(completion.content);
        if (isContextError) {
          ...
          if (retryResult.finishReason !== "error") {
            ...
            return { status: "completed", dataUpdate: { details: retryResult.content, response: retryResult.content }, charts };
          }
        }
      ...
      return { status: "completed", dataUpdate: { details: completion.content, response: completion.content }, charts };
    },
  });
...
function fallbackFailureAnalysis(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const summary =
    failuresArr.length === 0
      ...
      : `Found ${failuresArr.length} failures in ${results.length} tests (${Math.round((failuresArr.length / results.length) * 100)}% failure rate). Top categories: ${categoryBreakdown.slice(0, 3).map(([c, s]) => `${c} (${s.failed}/${s.total})`).join("; ")}.`;
  ...
  const chartBlock = charts.map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
      ...
      failuresArr.length > 0
        ? [
            ...
            categoryBreakdown.length > 0
              ? `Focus on ${categoryBreakdown[0]?.[0]} category`
              : "",
...
function fallbackFlakyDetection(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const summary =
    flaky.length === 0
      ...
      : `Found ${flaky.length} flaky tests across ${lookback} runs. Top: ${flaky.slice(0, 3).map((f) => `${f.id} (${f.score}% flakiness)`).join(", ")}.`;

  const chartBlock = [flakyChart, flakyTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
...
function fallbackAnomalyDetection(): AIAnalysisResult {
  ...
  const summaryChart = buildTable(
    ...
    [
      ...
      [
        "Below average",
        anomalies.filter((a) => a.flag === "below average").length,
      ],
      [
        "Above average",
        anomalies.filter((a) => a.flag === "above average").length,
      ],
    ],
  );

  const chartBlock = [summaryChart, passRateChart, anomalyTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    recommendations: anomalies
      ...
      .map((a) => `Investigate run ${a.runId} (${a.passPct}% pass rate — significantly below mean)`),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackRiskScoring(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const scoreTable = buildTable(
    ...
    [
      [`Pass rate (${run.passPct}%)`, "40%", `${Math.round(passRateScore)} pts`, `${run.passPct}% pass rate`],
      [`Failures (${run.failures})`, "20%", `${Math.round(failureScore)} pts`, `${run.failures} failures`],
      ["Trend", "20%", `${trendScore} pts`, prevRun ? `${prevRun.passPct}% → ${run.passPct}%` : "No previous run"],
  ...
  const chartBlock = [verdictTable, scoreChart, scoreTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    recommendations:
      level === "LOW"
        ...
        : level === "MEDIUM"
          ? [`Investigate ${run.failures} failures before deploy`, "Check if failures are env-specific"]
          : ["BLOCKING: Do not deploy", `Critical: ${run.failures} failures at ${run.passPct}% pass rate`, "Roll back to previous build"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackEnvComparison(): AIAnalysisResult {
  ...
  const chartBlock = [envChart, envTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    recommendations:
      envLabels.length >= 2 && envGroups[envLabels[0]]?.avgPass !== envGroups[envLabels[1]]?.avgPass
        ? ["Investigate environment-specific failures", "Check configuration parity between environments"]
...
function fallbackCategoryHealth(): AIAnalysisResult {
  ...
  const healthScores = Object.entries(allResults)
    .map(([cat, s]) => ({
      ...
      testCount: s.total,
      health: s.passed / s.total >= 0.95 ? "healthy" : s.passed / s.total >= 0.85 ? "fair" : "unhealthy",
    }))
  ...
  const healthTable = buildTable(
    ...
    healthScores.map((h) => [h.category, `${h.passRate}%`, h.testCount, `${h.avgDuration}ms`, h.health]),
  );
  ...
  const chartBlock = [summaryTable, healthChart, healthTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    recommendations: [
      ...
      ...(unhealthy.length > 0 ? ["Schedule focused review for unhealthy categories"] : ["All categories healthy — continue monitoring"]),
    ],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackCoverageGap(): AIAnalysisResult {
  ...
  const chartBlock = [coverageTable, gapTable, priorityTable, severityTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = gaps.length === 0 ? "Good coverage across all dimensions." : `Found ${gaps.length} gaps: ${gaps.join("; ")}.`;
  ...
  return {
    ...
    data: { testStats, gaps },
    confidence: 1,
    recommendations: gaps.length > 0 ? gaps.map((g) => `Address: ${g}`) : ["Coverage is adequate", "Focus on maintaining current coverage levels"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackSmartAlerting(): AIAnalysisResult {
  ...
  if (latest && latest.failures > 0)
    alerts.push({ severity: latest.failures > 10 ? "critical" : "warning", title: `${latest.failures} failures in latest run`, description: `Run ${latest.id} (${latest.build}) has ${latest.failures} failures at ${latest.passPct}% pass rate.` });
  if (latest && prev && latest.passPct < prev.passPct - 10)
    alerts.push({ severity: "critical", title: "Significant pass rate drop", description: `Pass rate dropped from ${prev.passPct}% to ${latest.passPct}%` });
  if (latest && prev && latest.durationMs > prev.durationMs * 1.5)
    alerts.push({ severity: "warning", title: "Duration spike detected", description: `Run duration increased from ${prev.durationMs}ms to ${latest.durationMs}ms.` });
  const lowScoreRuns = sortedRuns.filter((r) => r.passPct < 80);
  if (lowScoreRuns.length > 1)
    alerts.push({ severity: "warning", title: `${lowScoreRuns.length} runs below 80% pass rate`, description: lowScoreRuns.map((r) => `Run ${r.id}: ${r.passPct}%`).join(", ") });
  ...
  const chartBlock = [summaryTable, alertTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = alerts.length === 0 ? "No active alerts. All systems nominal." : `${alerts.length} active alert(s): ${alerts.filter((a) => a.severity === "critical").length} critical, ${alerts.filter((a) => a.severity === "warning").length} warnings.`;
...
function fallbackRunFrequency(): AIAnalysisResult {
  ...
  const chartBlock = [summaryTable, freqChart, freqTable, gapTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    confidence: 1,
    recommendations: gaps.length > 0 ? ["Fill coverage gaps with additional runs", "Consider scheduled runs on missed days"] : ["Good run frequency", "Maintain current schedule"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackSuiteHealth(): AIAnalysisResult {
  ...
  const suiteHealth = suites.map((suite) => {
    ...
    return { id: suite.id, name: suite.name, passRate: totalTests > 0 ? Math.round((passedTests / totalTests) * 100) : 0, totalExecutions: totalTests, config: suite.config };
  }).sort((a, b) => a.passRate - b.passRate);
  ...
  const chartBlock = [healthChart, healthTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = suiteHealth.length === 0 ? "No suite data available." : `Suite health across ${suiteHealth.length} suites. Unhealthiest: ${suiteHealth[0]?.name} (${suiteHealth[0]?.passRate}%).`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: suiteHealth.filter((s) => s.passRate < 85).map((s) => `Investigate ${s.name} (${s.passRate}% pass rate)`),
    generatedAt: new Date().toISOString(),
  };
}

function fallbackQualityGate(): AIAnalysisResult {
  const sortedRuns = [...RUNS].sort((a, b) => new Date(b.started).getTime() - new Date(a.started).getTime());
  ...
  const chartBlock = [summaryTable, gateTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = allPassed ? "Quality gate PASSED — all checks met." : `Quality gate FAILED — ${checks.filter((c) => !c.passed).length}/${checks.length} checks failed.`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: allPassed ? ["Gate passed — proceed with deployment"] : [`Fix: ${checks.filter((c) => !c.passed).map((c) => c.name).join(", ")}`, "Re-run after fixes"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackDurationBudget(): AIAnalysisResult {
  ...
  const chartBlock = [slowChart, slowTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = slowTests.length === 0 ? "No tests exceed duration budgets." : `${slowTests.length} tests exceed 1000ms average duration. Slowest: ${slowTests[0]?.id} (avg ${slowTests[0]?.avg}ms).`;
...
function fallbackRedundancyDetection(): AIAnalysisResult {
  ...
  const chartBlock = [redunTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = redundancies.length === 0 ? "No potentially redundant tests detected." : `Found ${redundancies.length} potential redundancy groups across ${new Set(redundancies.map((r) => r.category)).size} categories.`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: redundancies.slice(0, 3).map((r) => `Review ${r.tests.length} tests in ${r.category} for consolidation`),
    generatedAt: new Date().toISOString(),
  };
}

function fallbackRegressionPrediction(): AIAnalysisResult {
  const sortedRuns = [...RUNS].sort((a, b) => new Date(a.started).getTime() - new Date(b.started).getTime());
  ...
  const atRisk = Object.entries(categoryTrends).filter(([, t]) => t.decline && t.declinePct > 5).sort(([, a], [, b]) => b.declinePct - a.declinePct);
  ...
  const riskTable = buildTable(
    ...
    atRisk.map(([c, t]) => [c, `${t.rates[0]}% → ${t.rates[t.rates.length - 1]}%`, `${t.declinePct}%`]),
  );

  const chartBlock = [riskChart, riskTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = atRisk.length === 0 ? "No categories showing regression risk." : `${atRisk.length} category(s) at risk of regression.`;
  ...
  return {
    ...
    data: { atRisk: atRisk.map(([c, t]) => ({ category: c, declinePct: t.declinePct })), totalCategories: Object.keys(categoryTrends).length },
    confidence: 1,
    recommendations: atRisk.length > 0 ? atRisk.map(([c]) => `Investigate declining pass rate in ${c}`) : ["No regression risk detected — continue monitoring"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackPerformanceTrends(): AIAnalysisResult {
  ...
  const trending = Object.entries(testDurations)
    .map(([id, d]) => {
      const avg = d.durations.length > 0 ? Math.round(d.durations.reduce((s, v) => s + v, 0) / d.durations.length) : 0;
  ...
  const chartBlock = [perfChart, perfTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = slowing.length === 0 ? "No performance regressions detected." : `${slowing.length} test(s) slowing down. Worst: ${slowing[0]?.id} (+${slowing[0]?.change}%).`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: slowing.slice(0, 3).map((t) => `Investigate duration increase in ${t.id} (+${t.change}%)`),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackRootCauseAnalysis(): AIAnalysisResult {
  ...
  const byFirstSeen: Record<string, { test: string; category: string; firstRun: string; firstBuild: string; reappearances: number }> = {};
  for (const f of failures) {
    for (const run of sortedRuns) {
      ...
      if (match && match.status === "FAIL") {
        if (!byFirstSeen[f.id]) {
          byFirstSeen[f.id] = { test: f.id, category: f.category || "Unknown", firstRun: run.id, firstBuild: run.build || "", reappearances: 0 };
        } else {
  ...
  const systemic = Object.values(byFirstSeen).filter((b) => b.reappearances > 1).sort((a, b) => b.reappearances - a.reappearances);
  ...
  const chartBlock = [summaryTable, rootTable, newTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = failures.length === 0 ? `No failures in latest run (${latestRun.id}).` : `${failures.length} failures in latest run. ${systemic.length} systemic, ${recentFailures.length} new.`;

  return {
    useCaseId: "root-cause-analysis",
    summary,
    details: `## Root Cause Analysis — ${latestRun.id} (\`${latestRun.build}\`)\n\n${chartBlock}`,
    data: { systemic, newFailures: recentFailures, totalFailures: failures.length, latestBuild: latestRun.build },
    confidence: 1,
    recommendations: [...systemic.slice(0, 3).map((s) => `Root-cause recurring failure in ${s.test} (seen ${s.reappearances + 1} times)`), ...(failures.length === 0 ? ["No action needed"] : [])],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackFailureClustering(): AIAnalysisResult {
  ...
  const clusters: Record<string, { category: string; tests: string[]; count: number; pct: number }> = {};
  for (const f of failures) {
    const key = `${f.category || "Unknown"}::${f.suite || "none"}`;
    if (!clusters[key]) clusters[key] = { category: f.category || "Unknown", tests: [], count: 0, pct: 0 };
    clusters[key].tests.push(f.name);
    clusters[key].count++;
  }
  const total = failures.length;
  for (const c of Object.values(clusters)) c.pct = total > 0 ? Math.round((c.count / total) * 100) : 0;
  ...
  const chartBlock = [clusterPie, clusterTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = failures.length === 0 ? "No failures to cluster." : `${sortedClusters.length} cluster(s) across ${failures.length} failures.`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: sortedClusters.slice(0, 3).map((c) => `Investigate ${c.category} cluster (${c.count} failures)`),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackCrossCategoryCorrelation(): AIAnalysisResult {
  ...
  for (let i = 0; i < cats.length; i++) {
    for (let j = i + 1; j < cats.length; j++) {
      ...
      const diffs = aTrim.map((v, idx) => Math.sign(v - (aTrim[idx - 1] || v)) === Math.sign(bTrim[idx] - (bTrim[idx - 1] || bTrim[0])));
      const sameDirection = diffs.filter(Boolean).length;
      const strength = Math.round((sameDirection / diffs.length) * 100);
      if (strength > 60) correlations.push({ a: cats[i], b: cats[j], moveTogether: strength > 75, strength });
    }
  }
  ...
  const corrTable = buildTable(
    ...
    correlations.map((c) => [c.a, c.b, c.moveTogether ? "✅ Same direction" : "⚠️ Partial", `${c.strength}%`]),
  );

  const chartBlock = [corrTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = correlations.length === 0 ? "No significant correlations found." : `Found ${correlations.length} correlated pair(s). Strongest: ${correlations[0]?.a} ↔ ${correlations[0]?.b} (${correlations[0]?.strength}%).`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: correlations.length > 0 ? [`Monitor ${correlations[0]?.a} as leading indicator for ${correlations[0]?.b}`] : ["Add more runs to detect correlation patterns"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackTrendForecasting(): AIAnalysisResult {
  const sortedRuns = [...RUNS].sort((a, b) => new Date(a.started).getTime() - new Date(b.started).getTime());
  if (sortedRuns.length < 3) {
    const minTable = buildTable("Forecast Status", ["Metric", "Value"], [["Runs available", sortedRuns.length], ["Minimum required", 3]]);
    const j = JSON.stringify({ type: minTable.type, title: minTable.title, headers: minTable.headers, rows: minTable.rows, colors: minTable.colors, options: minTable.options });
    return {
  ...
  const forecastChart = buildLineChart(
    ...
    ["#5b8af5"],
    { vAxis: { minValue: 0, maxValue: 100 }, trendline: { type: "linear", color: "#ef4444", lineWidth: 2 } },
  );
  ...
  const forecastTable = buildTable(
    ...
    ["Metric", "Value"],
    [
      ["Trend", `${slope > 0 ? "improving" : slope < 0 ? "declining" : "stable"} (slope: ${slope.toFixed(2)})`],
  ...
  const chartBlock = [forecastTable, forecastChart].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    summary,
    details: `## Trend Forecasting\n\n${chartBlock}`,
    data: { trend, slope: Math.round(slope * 100) / 100, currentRate: rates[n - 1], forecast: next5 },
    confidence: 1,
    recommendations: trend === "declining" ? ["Investigate declining pass rate trend", "Review recent build changes", "Increase test frequency"] : trend === "improving" ? ["Continue current practices", "Monitor for trend reversal"] : ["No action needed — rates stable"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackFailureImpact(): AIAnalysisResult {
  ...
  for (const suite of suites) {
    ...
    if (failed.length > 0) impactedSuites.push({ id: suite.id, name: suite.name, failedTests: failed.map((f) => f.name) });
  }
  ...
  const chartBlock = [impactTable, impactCatTable, impactSuiteTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = failures.length === 0 ? "No failures in latest run." : `${failures.length} failure(s) impacting ${impactedSuites.length} suite(s) and ${Object.keys(byCategory).length} categories.`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: failures.length > 0 ? [...impactedSuites.slice(0, 2).map((s) => `Review failures blocking suite: ${s.name}`), ...(failures.length > 5 ? ["Blast radius is significant — consider rollback"] : [])] : ["No impact detected"],
    generatedAt: new Date().toISOString(),
  };
}

function fallbackEnvDrift(): AIAnalysisResult {
  const envGroups: Record<string, { runs: number; avgPass: number; failures: number; categories: Record<string, { total: number; passed: number }> }> = {};
  for (const run of RUNS) {
    const key = run.envId;
  ...
  const driftPairs: { a: string; b: string; passDiff: number; categories: { cat: string; aRate: number; bRate: number; diff: number }[] }[] = [];
  for (let i = 0; i < envKeys.length; i++) {
    for (let j = i + 1; j < envKeys.length; j++) {
      ...
      driftPairs.push({ a: envKeys[i], b: envKeys[j], passDiff: Math.abs(a.avgPass - b.avgPass), categories: catDiffs });
    }
  }
  ...
  const catDriftTables = significantDrift.flatMap((d) =>
    d.categories.length > 0
      ? [buildTable(`${d.a} vs ${d.b} — Category Drift`, ["Category", `${d.a}`, `${d.b}`, "Diff"], d.categories.map((c) => [c.cat, `${c.aRate}%`, `${c.bRate}%`, `${c.diff}%`]))]
      : [],
  );

  const chartBlock = [driftTable, ...catDriftTables].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

  const summary = significantDrift.length === 0 ? "No significant environment drift detected." : `${significantDrift.length} drift pair(s) detected.`;
  ...
  return {
    ...
    confidence: 1,
    recommendations: significantDrift.flatMap((d) => [...d.categories.slice(0, 2).map((c) => `Check config parity for ${c.cat} between ${d.a} and ${d.b}`), d.passDiff > 10 ? `Large drift (${d.passDiff}%) between ${d.a} and ${d.b}` : ""]).filter(Boolean),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackBuildRiskAssessment(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const totalScore = Math.round(passRateScore + failureScore + trendScore + envScore);
  const level = totalScore <= 20 ? "LOW" : totalScore <= 50 ? "MEDIUM" : totalScore <= 80 ? "HIGH" : "CRITICAL";
  const verdict = level === "LOW" ? "✅ Safe to deploy" : level === "MEDIUM" ? "⚠️ Proceed with caution" : "❌ Do not deploy";
  ...
  const buildTable_ = buildTable(
    ...
    [
      ...
      ["Trend", "25%", `${trendScore} pts`, prevRun ? `${prevRun.passPct}% → ${latest.passPct}%` : "No previous run"],
      ["Environment", "15%", `${envScore} pts`, latest.env],
    ],
  );

  const chartBlock = [buildChart, buildTable_].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    data: { build: latest.build, riskScore: totalScore, riskLevel: level, verdict, passPct: latest.passPct, failures: latest.failures },
    confidence: 1,
    recommendations: level === "LOW" ? [`Build ${latest.build} approved for deployment`] : level === "MEDIUM" ? [`Review ${latest.failures} failures before deploying`, "Run additional smoke tests"] : [`BLOCK: Do not deploy ${latest.build}`, `Critical: ${latest.failures} failures at ${latest.passPct}% pass rate`, "Roll back to previous stable build"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackPromotionDecisionSupport(): AIAnalysisResult {
  ...
  if (!latestUat) {
    const noDataTable = buildTable("Promotion Status", ["Check", "Value"], [["UAT runs", "0"], ["Status", "No UAT data — cannot evaluate"]]);
    const j = JSON.stringify({ type: noDataTable.type, title: noDataTable.title, headers: noDataTable.headers, rows: noDataTable.rows, colors: noDataTable.colors, options: noDataTable.options });
    return {
  ...
  const checks = [
    ...
    { name: "UAT failures < 5", passed: latestUat.failures < 5, actual: `${latestUat.failures} failures` },
    { name: "UAT/PROD parity (≤5% diff)", passed: prodParity, actual: latestProd ? `${Math.abs(latestUat.passPct - latestProd.passPct)}% diff` : "No PROD data" },
  ];
  const allPassed = checks.every((c) => c.passed);
  ...
  const chartBlock = [statusTable, promoTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    data: { decision, checks, latestUat: latestUat.id, uatPassPct: latestUat.passPct, uatBuild: latestUat.build },
    confidence: 1,
    recommendations: decision === "PROMOTE" ? ["Promote to PROD — all checks pass", "Monitor first PROD run after promotion"] : decision === "PENDING" ? ["Review failing checks and retry", "Check env-specific configuration differences"] : ["BLOCKED: UAT pass rate below 95% threshold", "Fix UAT failures before promotion attempt"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackTestDocGen(): AIAnalysisResult {
  const testCases = getTestCases();
  const suites = getTestSuites();
  const byCategory: Record<string, { count: number; automated: number; manual: number; priorities: string[] }> = {};
  for (const tc of testCases) {
    const cat = tc.category || "Unknown";
    ...
    if (tc.priority && !byCategory[cat].priorities.includes(tc.priority)) byCategory[cat].priorities.push(tc.priority);
  }
  const testInventory = Object.entries(byCategory).map(([cat, s]) => ({
    category: cat, total: s.count, automated: s.automated, manual: s.manual,
  ...
  const docTable = buildTable(
    ...
    testInventory.map((c) => [c.category, c.total, c.automated, c.manual, `${c.coverage}%`, c.priorities]),
  );
  ...
  const chartBlock = [overviewTable, docTable, suiteTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    confidence: 1,
    recommendations: testInventory.filter((c) => c.coverage < 50).map((c) => `Increase automation in ${c.category} (${c.coverage}% coverage)`),
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackReleaseReadiness(): AIAnalysisResult {
  ...
  const overallScore = Math.round(latest.passPct * 0.4 + (healthyEnvs / Math.max(envCount, 1)) * 100 * 0.3 + Math.max(0, 100 - latest.failures * 5) * 0.3);
  ...
  const chartBlock = [releaseTable, envTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    confidence: 1,
    recommendations: verdict === "✅ RELEASE READY" ? [`Build ${latest.build} approved for release`] : verdict === "⚠️ CAUTION" ? [`Review environments with failures`, `Check latest run: ${latest.failures} failures`] : [`RELEASE BLOCKED: ${latest.failures} failures at ${latest.passPct}% pass rate`, "Fix blocking issues before release attempt"],
    generatedAt: new Date().toISOString(),
  };
}
...
function fallbackRegressionReport(): AIAnalysisResult {
  ...
  if (!latest || !prev) {
    const noDataTable = buildTable("Regression Status", ["Metric", "Value"], [["Runs available", RUNS.length], ["Minimum required", 2]]);
    const j = JSON.stringify({ type: noDataTable.type, title: noDataTable.title, headers: noDataTable.headers, rows: noDataTable.rows, colors: noDataTable.colors, options: noDataTable.options });
    return {
  ...
  for (const [id, lr] of latestMap) {
    ...
    if (!pr) continue;
    if (pr.status === "PASS" && lr.status === "FAIL") { regressed++; regressedTests.push({ id, name: lr.name, from: pr.status, to: lr.status }); }
    else if (pr.status === "FAIL" && lr.status === "PASS") { improved++; improvedTests.push({ id, name: lr.name, from: pr.status, to: lr.status }); }
    else { same++; }
  }
  ...
  const chartBlock = [regSummaryTable, regTable, impTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");
  ...
  return {
    ...
    confidence: 1,
    recommendations: regressed > 0 ? [`Investigate ${regressed} regressed test(s)`, `Run diff between ${prev.build} and ${latest.build}`] : ["No regressions — good to proceed"],
    generatedAt: new Date().toISOString(),
  };
}
...
function genericFallback(request: AIAnalysisRequest): AIAnalysisResult {
  ...
  const j = JSON.stringify({ type: genericTable.type, title: genericTable.title, headers: genericTable.headers, rows: genericTable.rows, colors: genericTable.colors, options: genericTable.options });
  return {
    useCaseId: request.useCaseId,
    ...
    confidence: 0.5,
    recommendations: ["Configure OpenAI or Chrome AI for full analysis", "Use specific analysis types (failure-analysis, flaky-detection, etc.)", "Data is available for all standard queries"],
    generatedAt: new Date().toISOString(),
  };
}
...
function extractRecommendations(text: string): string[] {
  ...
  for (const line of lines) {
    ...
    else if (inRecs && line.trim().startsWith("-")) recommendations.push(line.trim().replace(/^-\s*/, ""));
    else if (inRecs && line.trim().startsWith("•")) recommendations.push(line.trim().replace(/^•\s*/, ""));
    else if (inRecs && line.trim().startsWith("1.")) recommendations.push(line.trim().replace(/^\d+\.\s*/, ""));
  ...
  if (sortedRuns.length >= 2) {
    ...
    if (prev && latest.passPct < prev.passPct - 10) {
      insights.push({
        ...
        category: "trend",
        severity: 85,
        relatedData: { from: prev.passPct, to: latest.passPct, fromBuild: prev.build, toBuild: latest.build },
        timestamp: new Date().toISOString(),
      });
    }
  }
...
function fallbackSetupGuide(): AIAnalysisResult {
  ...
  const stepsTable = buildTable("Quick Start", ["#", "Step"], topics.map((t, i) => [i + 1, t]));
  ...
  const chartBlock = [stepsTable, secretsTable, errorsTable].map((c) => {
    const j = JSON.stringify({ type: c.type, title: c.title, headers: c.headers, rows: c.rows, colors: c.colors, options: c.options });
    return `\`\`\`chart\n${j}\n\`\`\``;
  }).join("\n\n");

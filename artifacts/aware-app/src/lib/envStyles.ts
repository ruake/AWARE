...
export function envBadgeClass(env: string): string {
  return (
    ENV_BADGE_CLASS[env] ??
    "bg-gcp-text-muted/15 text-gcp-text-secondary border border-gcp-border/25"
  );
}

export function envSelectClass(env: string): string {
  return (
    ENV_SELECT_CLASS[env] ?? "bg-gcp-text-muted/20 text-gcp-text-secondary border-gcp-text-muted/30"
  );
}
...
export function passRateColor(pct: number): { text: string; bar: string; glow: string } {
  if (pct >= 95)
    return { text: "text-gcp-green", bar: "bg-gcp-green", glow: "rgba(52,168,83,0.4)" };
  if (pct >= 80)
    return { text: "text-gcp-yellow", bar: "bg-gcp-yellow", glow: "rgba(251,188,5,0.4)" };

...
:link: <${e.url}|View in A.W.A.K.E.>  |  :spiral_note_pad: <${e.url}/evidence|Evidence>`;

  const githubBody = `## A.W.A.K.E. ${e.status === "REGRESSION" ? "Regression" : "Test Failure"} Report
...
---
_Auto-generated by [A.W.A.K.E.](${BASE}) · ${new Date().toISOString().slice(0, 10)}_`;

  const jiraDesc = `h2. A.W.A.K.E. Alert
...
|Context|${e.title}|
|Detail|${e.meta}|
|Link|[View in A.W.A.K.E.|${e.url}]|

h3. Steps to Reproduce
# Open A.W.A.K.E. Run Detail: ${e.url}
# Review evidence panel for request/response diff
# Cross-reference PM variables section`;

  const emailSubj = `[A.W.A.K.E.] ${e.status}: ${e.id}`;
...
--
Sent by A.W.A.K.E. (Akamai Web Analyser & Kit for Evaluations)`;

  const badge = BADGES[activeBadge];
  const badgeHtml = `<img src="${BASE}/badges/${badge.target.toLowerCase().replace(/\//g, "-").replace(/\s/g, "")}.svg"
     alt="A.W.A.K.E. ${badge.target}: ${badge.pass}%"
     title="${badge.target} — ${badge.pass}% pass rate" />`;
  const badgeMd = `![A.W.A.K.E. ${badge.target}](${BASE}/badges/${badge.target.toLowerCase().replace(/\//g, "-").replace(/\s/g, "")}.svg)`;
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[13px] text-[var(--gcp-text-secondary)] mt-0.5">
              Deep links, export formats, and embeddable badges for every entity in A.W.A.K.E.
            </p>
          </div>
                ...
                {BADGES.map((b, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-center text-white text-[11px] font-medium rounded overflow-hidden shadow-sm shrink-0">
                      <span className="bg-[#555] px-2 py-0.5 whitespace-nowrap">A.W.A.K.E.</span>
